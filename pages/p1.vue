<template>
  <div>
    <div class="container is-flex-desktop justify-center item-wrap">
      <div class="left-desc">
        <div>
          <img src="~/assets/p1/i_pd.png" alt>
        </div>
        <div class="lock-desc">如此快捷,一握即开</div>
        <p class="desc fs-24px">活体指纹识别 | 瑞典FPC芯片 | 精致外观工艺 | 人工智能算法</p>
        <div class="icons flex">
          <div class="has-text-centered">
            <img src="~/assets/p1/icon_zwsb.png" style="magin:auto;" alt>
            <div class="fs-16px">活体指纹识别</div>
          </div>
          <div class="icon-item has-text-centered">
            <img src="~/assets/p1/i_2.png" style="magin:auto;" alt>
            <div class="fs-16px">一键开启</div>
          </div>
          <div class="icon-item has-text-centered">
            <img src="~/assets/p1/i_3.png" style="magin:auto;" alt>
            <div class="fs-16px">智能算法</div>
          </div>
          <div class="icon-item has-text-centered">
            <img src="~/assets/p1/i_4.png" alt>
            <div class="fs-16px">双轴承防下挂</div>
          </div>
          <div class="icon-item has-text-centered">
            <img src="~/assets/p1/i_5.png" alt>
            <div class="fs-16px">安全把控</div>
          </div>
        </div>
      </div>
      <div class="pic-neck has-text-centered">
        <img class="pic-m1" src="~/assets/index/p1.png" alt>
      </div>
    </div>
    <div class="bg-1">
      <div class="container">
        <div class="word-title has-text-centered">
          <h2>活体指纹识别.开锁更安全</h2>
          <p class="fs-26px">
            高性能活体半导体指纹技术，360°圆形指纹头触感体验，更方便、更快捷。
            采用防指纹表面处理，不留指纹，更安全；不留污渍，更清洁。
          </p>
        </div>
      </div>
    </div>
    <div class="bg-2">
      <div class="container">
        <div class="word-title has-text-centered">
          <h2>人工智能算法，一键开启</h2>
        </div>
        <div style="width: 40%;margin-left:50%;color:#fff;">
          <p class="fs-32px">
            可以自主学习的算法，在日常使用中
            对干湿、残缺指纹识别度提升。
          </p>
          <p class="fs-26px" style="margin:32px 0 82px;">
            还会随着使用次数的增加，对用户指纹识别
            越来越精确降低拒真率，一键开门。
          </p>
          <div class="fs-20px">
            指纹识别精确度
            <span class="up-arrow">↑</span>
          </div>
          <div class="percent">96.5%</div>
        </div>
      </div>
    </div>
    <div class="bg-3">
      <div class="container">
        <div class="word-title has-text-centered">
          <h2>双轴承设计 防止下挂</h2>
          <p class="fs-26px">
            采用来自德国的双轴承结构，使把手不晃动，不下垂，下压把手开门柔顺，不卡顿，每天使用开门30次+，
            使用10年依然不下垂。
          </p>
        </div>
        <div class="c-fff icon-tip">
          <div class="circle-c c-fff flex justify-center items-center">C级</div>
          <div class="fs-26px">锁芯等级</div>
        </div>
      </div>
    </div>

    <div class="bg-5" style="padding-top: 71px">
      <div class="word-title has-text-centered">
        <h2>虚位密码</h2>
        <p class="fs-26px" style="margin: 70px 0 30px;">
          可有效防止他人通过指纹查看密码痕迹破解密码（比如不方便让同行的亲友看到正确密码，则可
          以输入虚位密码，避免尴尬）
        </p>
        <h3 class="fs-28px" style="margin: 0px 0 30px;">使用方法</h3>
        <div class="flex justify-center">
          <img src="~/assets/m1/5_1.png" alt>
        </div>
        <p class="fs-20px" style="margin: 20px 0 0px;">15位数字内含正确密码</p>
      </div>
    </div>
    <div class="bg-4">
      <div class="container">
        <div class="word-title has-text-left" style="width: 391px">
          <h2>多种开锁方式 方便快捷</h2>
          <p class="fs-26px">
            指纹、密码、智能指纹锁感应卡、钥匙、多方式齐上阵，集科技与实用合二为一，无论何时何地都能轻松开门，
            让生活更便捷。
          </p>
          <div class="flex justify-between">
              <div>
                <img src="~/assets/p1/4_2.png" alt>
                <div>智能指纹锁感应卡解锁</div>
              </div>
              <div>
                <img src="~/assets/p1/4_2.png" alt>
                <div>指纹解锁</div>
              </div>
              <div>
                <img src="~/assets/p1/4_2.png" alt>
                <div>数字密码解锁</div>
              </div>
              <div>
                <img src="~/assets/p1/4_2.png" alt>
                <div>钥匙解锁</div>
              </div>
          </div>
        </div>
      </div>
    </div>
    <div>
      <div class="bg-6"></div>
    </div>
  </div>
</template>

<style lang="less" scoped>
img {
  display: block;
}
@media screen and (min-width: 1024px) {
  .container {
    font-size: 0;
    padding-top: 109px;
    justify-content: center;
    // width: 513px;
    .left-desc {
      padding-top: 100px;
      margin-right: 62px;
      width: 700px;
      .icon-item {
        margin-left: 90px;
      }
    }
  }

  .bg-1 {
    height: 1079px;
  }
  .bg-2 {
    height: 1106px;
    .up-arrow {
      color: #1670eb;
      font-size: 28px;
    }
    .percent {
      font-size: 72px;
      color: #1670eb;
    }
  }
  .bg-3 {
    height: 1249px;
    .icon-tip {
      margin-left: 155px;
      .circle-c {
        font-size: 32px;
        z-index: 1000;
        border-radius: 50%;
        width: 89px;
        height: 89px;
        border: 2px solid rgba(255, 255, 255, 1);
        margin-bottom: 29px;
      }
    }
  }
  .bg-4 {
    height: 931px;
  }
  .bg-5 {
    height: 975px;
  }
  .bg-6 {
    height: 842px;
  }
}
@media screen and (max-width: 1023px) {
  .pic-m1 {
    margin-top: 30px;
  }
}
.item-wrap {
  margin-bottom: 120px;
}
.left-desc {
  .lock-desc {
    font-size: 60px;
    margin: 20px 0 24px;
  }
  .desc {
    margin-bottom: 58px;
  }
}
.pic-m1 {
  width: 370px;
  height: 666px;
}

@media screen and (max-width: 768px) {
  .section {
    .container {
    }
  }
  .pic-m1 {
    width: 185px;
    height: 333px;
  }
}

.bg-1 {
  background: url("../assets/p1/bg_1.png");
}
.bg-2 {
  background: url("../assets/p1/bg_2.png");
}
.bg-3 {
  background: url("../assets/p1/bg_3.png");
}
.bg-4 {
  background: url("../assets/p1/bg_4.png");
}
.bg-5 {
  background: url("../assets/m1/5_2.png");
}
.bg-6 {
  background: url("../assets/p1/bg_6.png");
  background-size: 100%;
}

.word-title {
  color: #ffffff;
  h2 {
    font-size: 60px;
    margin: 50px 0 40px;
  }
  p {
    margin-bottom: 87px;
  }
}
</style>
